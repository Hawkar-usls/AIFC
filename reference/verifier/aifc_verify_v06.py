#!/usr/bin/env python3
"""AIFC Verifier A standalone replay CLI v0.6.

v0.6 verifies direct Ed25519 signatures over the frozen typed preimages against
keys in the exact signing registry snapshot. Historical compromise and freshness
...
from full_admission_v06 import verify_replay_manifest
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="aifc-verify-v06",
        description=(
            "Replay AIFC evidence through the current stack, reconstruct exact typed "
            "signature messages, and cryptographically verify Ed25519 receipts. "
            "Historical key-lifecycle and freshness remain blocked."
        ),
    )
