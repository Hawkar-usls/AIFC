#!/usr/bin/env python3
"""AIFC Verifier A standalone replay CLI v0.5.

v0.5 freezes/replays the exact Ed25519 signature preimage but deliberately keeps
ED25519_SIGNATURE_CRYPTO blocked until the subsequent crypto gate.
"""
...
from full_admission_v05 import verify_replay_manifest
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="aifc-verify-v05",
        description=(
            "Replay AIFC evidence through the current pre-crypto stack, including "
            "the frozen typed Ed25519 signature-preimage profile. Ed25519 signature "
            "cryptography remains blocked and therefore process success is not a "
            "substitute for scientific admission."
        ),
    )
...
def invalid(detail: str) -> dict:
    return {
        ...
        "gate_results": {"CLI_INPUT": "FAIL", "ED25519_SIGNATURE_CRYPTO": "BLOCKED"},
        "exact_match": False,
