...
Strongest-grade replay derives protocol_version, content_schema and registry_sequence
from exact content-addressed evidence; they are not claimant inputs.
"""
...
def replay_signature_preimage(receipt: Mapping[str, Any], *, experiment_plan_hash: str, resolver) -> bytes:
    """Strongest-grade preimage replay with semantic metadata derived from evidence.

    No Ed25519 verification occurs here. The output is only the exact message bytes
    a later pure-Ed25519 gate is required to verify.
    """
    binding = receipt_binding(receipt)
